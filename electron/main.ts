...
function buildCounterSessionV2(
  mapName: string,
  routeMeta?: {
    source?: CounterRouteSource;
    confidence?: number;
...
function hasCounterSceneTemplateV2(data: AppData): boolean {
  return Boolean(data.settings.counterSceneTemplatePath.trim());
}

function getCounterRouteSourceLabelV2(source: CounterRouteSource | undefined): string {
  switch (source) {
    case 'template':
      return '模板识别';
    case 'history':
...
function inferPreferredCounterRouteFromHistoryV2(data: AppData): {
  mapName: string;
  source: CounterRouteSource;
  confidence?: number;
  detail: string;
...
function resolveCounterRouteV2(data: AppData, event: {
  routeName?: string;
  routeConfidence?: number;
  routeDetail?: string;
  preferredFallbackMapName?: string;
...
async function handleRunCounterStateEventV2(event: {
  ...
  session.lastRouteTemplate = event.routeTemplate?.trim() || '';
  session.lastRouteThreshold = event.routeRequiredConfidence;
  const route = resolveCounterRouteV2(
    data,
    {
      routeName: event.routeName,
      routeConfidence: event.routeConfidence,
